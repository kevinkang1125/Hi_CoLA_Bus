...
def cb_sliding_window_search(traj_list,return_list,eval_policy,base_policy,min_epoch,max_epoch,step,converge,confidence_level):
    ...
    for epochs in tqdm(range(min_epoch, max_epoch, step), desc="Processing Epochs"):
        ...
        iters.append(epochs)
        if epochs >100000:
            if np.var(lower_bound_list[-30:])<converge*np.mean(lower_bound_list[-30:]):
                break
        
    return epochs,lower_bound
...
def Thomas_hc(behavior_pi, eval_policy, traj_list, return_list, confidence_level=0.9):
    ...
    lower_bound = cb_sliding_window_search(traj_list,return_list,eval_policy,base_policy,min_epoch=min_epochs,max_epoch=max_epochs,step=step,converge=converge,confidence_level=confidence_level)
