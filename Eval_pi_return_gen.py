...
def return_collect(traj_num, gamma, base_policy, batch_size=1000, pi_id = 0):
    all_return = []
    ...
    with ProcessPoolExecutor() as executor:
        for batch_start in tqdm(range(0, traj_num, batch_size), desc="Processing batches"):
            ...
            for traj_id, (ret) in enumerate(results):
                all_return.append(ret)

    # df = pd.DataFrame(all_rows)
    #df.to_parquet("./Traject/trajectories.pq")
    np.savetxt(f"./Test/return_list_{pi_id}.txt", all_return)
...
if __name__ == "__main__":
    ...
    return_list = return_collect(traj_num=traj_num,gamma=gamma,base_policy=pi,pi_id = i)
