...
class OptimizedTrainer:
    ...
    def optimized_training_loop_fatigue(self, behavior_pi, hi_cola, target_lb, env_name, traj_num, gamma, 
                                states, b_loop,threshold = 1, max_steps=100, lr=0.00005):
            ...
            for step in range(max_steps):
                ...
                if kl_between_policies(behavior_pi, start_pi, test_states).item() > 0.1:
                    break
                elif confidence_lb.item() > threshold:
                    break
...
def main():
    ...
    while not done:
        ...
        output_pi, return_trace, model_trace = trainer.optimized_training_loop_fatigue(
            behavior_pi, hi_cola, target_lb, env_name, traj_num, gamma, states,b_loop,threshold = label_up
        )
        ...
        param_drift.to_csv(f"./exp_multi_epochs/changed_object2/parameter_drift_{b_loop}.csv", index=False)
...
if __name__ == "__main__":
    ...
    torch.set_num_threads(min(6, os.cpu_count()))
