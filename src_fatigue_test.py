...
class OptimizedTrainer:
    ...
    def optimized_training_loop(self, behavior_pi, hi_cola, target_lb, env_name, traj_num, gamma, 
                               states, b_loop,threshold = 1, max_steps=100, lr=0.0004):
        ...
        for step in range(max_steps):
            ...
            np.savetxt(f"./exp_multi_epochs/dynamic_bounds/policy_differences_{b_loop}_{step}.txt", z_score_step, delimiter=",")
            ...
            # Periodic evaluation (every 5 steps to reduce overhead)
            if step % 2 == 0:
                ...
                # Calculate mean return and save returns
                # current_mean_return = np.mean(return_list)
                # mean_returns.append(current_mean_return)
                ...
                #     best_pi = copy.deepcopy(behavior_pi)
                # else:
                #     print(f"Performance decreased: previous best = {best_mean_return:.4f}, current = {current_mean_return:.4f}")
                #     output_pi = best_pi
                #     break
                
                # KL divergence check
                # evaluate_pi = copy.deepcopy(cpu_pi).to(self.device)
                # kl = kl_between_policies(evaluate_pi, start_pi, test_states).item()
                # kl_ls.append(kl)
            escaped,z_max= check_adaptive_perturbation_escape_75(
                behavior_pi, start_pi, c=0.15, threshold=threshold)
            print(f"z_max:{z_max}")
            z_max_rc.append(z_max)
            np.savetxt(f"./exp_multi_epochs/dynamic_bounds/z_max_track_{b_loop}.txt", z_max_rc, delimiter=",")            
            if escaped:
                print(f"Perturbation boundary exceeded (threshold>0.02)z_max:{z_max}. Stopping training.")
                break
            elif kl_between_policies(behavior_pi, start_pi, test_states).item() > 0.1:
                break
            
            output_pi = copy.deepcopy(behavior_pi)
            print(f"Step {step+1}/{max_steps}, Loss: {loss.item():.4f}, Confidence LB: {confidence_lb.item():.4f}")
        
        # Save mean returns history
        # np.savetxt(f"./exp_multi_epochs/dynamic_bounds/z_max_track_{b_loop}.txt", z_max_rc, delimiter=",")
        # print(f"Best mean return achieved: {best_mean_return:.4f}")
        # print(f"KL divergences: {kl_ls}")
    ...
    def optimized_training_loop_fatigue(self, behavior_pi, hi_cola, target_lb, env_name, traj_num, gamma, 
            ...
            for step in range(max_steps):
                ...
                train_pi = copy.deepcopy(behavior_pi)
                z_score_step = check_perturbation_range(train_pi,base_pi=start_pi,c=1.5)
                np.savetxt(f"./exp_multi_epochs/troubleshoot/policy_differences_{b_loop}_{step}.txt", z_score_step, delimiter=",")
                confidence_lb = hi_cola(input_vector)
                return_trace.append(confidence_lb.item())
                ...
                if step % 2 == 0:
                    ...
                    np.savetxt(f"./exp_multi_epochs/troubleshoot/raw_reward_{b_loop}_{step}.txt", return_list, delimiter=",")
...
def main():
    ...
    while not done:
        ...
        np.savetxt(f"./exp_multi_epochs/troubleshoot/label_up_record.txt",label_up_ls, delimiter=",")
        ...
        os.makedirs("./exp_multi_epochs/troubleshoot/", exist_ok=True)
        np.savetxt(f"./exp_multi_epochs/troubleshoot/Confidence_Lowerbound_Thomas_{b_loop}.txt", 
                  Thomas_gt, delimiter=",")
        np.savetxt(f"./exp_multi_epochs/troubleshoot/Confidence_Lowerbound_Predict_{b_loop}.txt", 
                  return_trace, delimiter=",")
        np.savetxt(f"./exp_multi_epochs/troubleshoot/Epoch_Record.txt", 
                  epoch_record, delimiter=",")
        np.savetxt(f"./exp_multi_epochs/troubleshoot/Epoch_Length_Record.txt", 
                  step_rec, delimiter=",")
        np.savetxt(f"./exp_multi_epochs/troubleshoot/r2_Record.txt", 
                  mr_rec, delimiter=",")
    ...
    torch.save(behavior_pi.state_dict(), 
              "./exp_multi_epochs/troubleshoot/Optimized_Behavioural_model.pth")
    ...
    plt.tight_layout()
    plt.savefig("./exp_multi_epochs/troubleshoot/training_progress.png", dpi=300)
    plt.show()
