...
def bootstrap_convergence(return_list, up_b, low_b, batch_size=1000, max_size=None, num_bootstrap=100, var_tol=1e-4, window=150):
    if max_size is None:
    ...
    for n in tqdm(range(batch_size, max_size + 1, batch_size), desc="Bootstrapping"):
        current_bounds = []

        for _ in range(num_bootstrap):
            sample = np.random.choice(return_list[:n], size=n, replace=True)
...
sizes_boot, bounds_boot = bootstrap_convergence(return_list, up_b, low_b)
